...
try {
  const capabilities = readTextJson(await callTool("handout_get_capabilities", {}));
  ...
  const sites = readTextJson(await callTool("handout_list_sites", {}));
  assert(Array.isArray(sites.sites), "list sites should return a sites array");
  const created = await callTool("handout_create_site", {
    name: `Agent smoke ${Date.now()}`,
  });
  const siteId = readTextJson(created).site.id as string;
  const content = await callTool("handout_get_site_content", { siteId });
  ...
  const updated = readTextJson(await callTool("handout_update_site_content", {
    siteId,
  ...
  const staleRevisionResult = readTextJson(await callTool("handout_update_site_content", {
    siteId,
    expectedDraftRevision: draftRevision,
    draftContent,
  }, { allowError: true }));
  ...
  const validation = readTextJson(await callTool("handout_validate_site_content", { siteId, draftContent }));
  assert(validation.valid === true, "site content should validate before publishing");
  const variants = readTextJson(await callTool("handout_batch_upsert_variants", {
    siteId,
    matchBy: "slug",
    variants: [
      {
  ...
  const duplicateVariantResult = readTextJson(await callTool("handout_batch_upsert_variants", {
    siteId,
    matchBy: "slug",
    variants: [
      {
  ...
  const updatedVariantById = readTextJson(await callTool("handout_batch_upsert_variants", {
    siteId,
    matchBy: "id",
    variants: [
      {
  ...
  const listedVariants = readTextJson(await callTool("handout_list_variants", { siteId }));
  assert(listedVariants.variants.length === 2, "variant list should return two variants");
  const site = readTextJson(await callTool("handout_get_site", { siteId }));
  assert(site.site.id === siteId, "get site should return the created site");
  const siteWithVisibility = readTextJson(await callTool("handout_update_site", {
    siteId,
    visibility: "team",
  }));
  assert(siteWithVisibility.site.visibility === "team", "site update should allow making the site browser-accessible");
  const published = readTextJson(await callTool("handout_publish_site", { siteId }));
  assert(published.site.status === "published", "publish should mark the site published");
  const publicUrls = readTextJson(await callTool("handout_get_public_urls", {
    siteId,
    workspaceSlug: "handout-dev",
    publicSiteOrigin: "http://localhost:3011",
  }));
  assert(typeof publicUrls.siteUrl === "string", "public URLs should include a site URL");
  const trackingSummary = readTextJson(await callTool("handout_get_tracking_summary", { workspaceId: devWorkspaceId, siteId }));
  assert(typeof trackingSummary.metrics === "object", "tracking summary should include metrics");
  const trackingEvents = readTextJson(await callTool("handout_list_tracking_events", {
  ...
  const trackingSessions = readTextJson(await callTool("handout_list_tracking_sessions", {
    workspaceId: devWorkspaceId,
    siteId,
    limit: 5,
  }));
  assert(Array.isArray(trackingSessions.sessions), "tracking sessions should return a sessions array");
  const unpublished = readTextJson(await callTool("handout_unpublish_site", { siteId }));
