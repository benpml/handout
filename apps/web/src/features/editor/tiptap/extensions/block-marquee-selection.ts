...
export const HandoutNextBlockMarqueeSelection = Extension.create({
  ...
  onDestroy() {
    ...
    blockMarqueeSelectionControllers.delete(this.editor.view)
    controller?.destroy()
  },
        ...
        props: {
          handleDOMEvents: {
            mousedown: (view, event) =>
              getOrCreateBlockMarqueeSelectionController(view).handleMouseDown(event),
            pointerdown: (view, event) =>
              event instanceof PointerEvent
                ? getOrCreateBlockMarqueeSelectionController(view).handlePointerDown(event)
                : false,
          },
        },
        view: (view) => {
          getOrCreateBlockMarqueeSelectionController(view)
          return {}
        },
      }),
    ]
...
function getOrCreateBlockMarqueeSelectionController(view: EditorView) {
  const existingController = blockMarqueeSelectionControllers.get(view)
