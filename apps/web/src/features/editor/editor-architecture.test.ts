...
describe("editor architecture", () => {
  ...
  it("uses the compact Figma header with icon preview, send, primary publish, and settings actions", () => {
    const headerSource =
      editorModules["./components/editor-header.tsx"] as string

    expect(headerSource).not.toContain('import { Tabs, TabsList, TabsTrigger }')
    expect(headerSource).not.toContain("EditorModeTabs")
    expect(headerSource).not.toContain("EditorPublishStatusBadge")
    expect(headerSource).not.toContain("EditorHistoryControls")
    expect(headerSource).toContain('h-[46px]')
    expect(headerSource).toContain('h-[30px]')
